...
export default function RoadmapPage() {
  ...
  const runRoadmap = useCallback(async () => {
    ...
    try {
      ...
      let validated: ImplementationRoadmap | null = null;
      ...
      await readNdjsonStream(response, (event) => {
        if (event.type === "partial") {
          ...
          validated = event.data as ImplementationRoadmap;
      ...
      if (!validated) {
        setErrorMessage(
          "Roadmap completed without a validated result. Please try again."
        );
        setStreamState("error");
        return;
      }
      ...
      setErrorMessage(
        "Failed to process the roadmap response. Please try again."
      );
      setStreamState("error");
    }
  }, [intake, results, setResults, markStepComplete]);
  ...
  useEffect(() => {
    ...
    if (results.roadmap && !hasFired.current) {
      setResult(results.roadmap);
      setStreamState("complete");
      hasFired.current = true;
      return;
    }
  ...
  }, [intake.agencyType, isHydrated, router, runRoadmap, results.roadmap]);
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm p-8">
          ...
          <div className="mt-6 rounded-lg border border-coral/10 bg-coral/5 p-4 flex items-start gap-3">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              ...
              and reflects publicly documented federal deployment pathways.
              For a production engagement, an Anthropic Solutions Architect
              would validate model-version authorization and refine this
